...
func main() {
	...
	fmt.Println("• SecondDegree   → document still wins, even after per-label indexing cut the")
	fmt.Println("                    graph from 87 to 47 hops. The graph walks EVERY connection edge")
	fmt.Println("                    in the 2-hop frontier (incl. back-edges + already-seen people),")
	fmt.Println("                    while the document enumerates neighbor IDs for free from the")
	fmt.Println("                    loaded blob and fetches each UNIQUE person once. Locality +")
	fmt.Println("                    dedup-before-fetch is hard to beat for bounded multi-hop.")
}
